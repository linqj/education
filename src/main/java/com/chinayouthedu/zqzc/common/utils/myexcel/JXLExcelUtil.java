			...
			if (rows > 0) {// 有数据时才处理
				...
				for (Field field : allFields) {
					...
					if (field.isAnnotationPresent(ExportExcel.class)) {
						ExportExcel attr = field
								.getAnnotation(ExportExcel.class);
				...
				for (int i = 1; i < rows; i++) {// 从第2行开始取数据,默认第一行是表头.
					...
					for (int j = 0; j < cells.length; j++) {
						...
						if(condition.length > 0){
							System.out.println("存在:"+c+"   对比0:   "+condition[0].toString()+"   对比1:   "+condition[1].toString());
							if(c.trim().equals(condition[0].trim().toString())){//true
						...
						if ((Integer.TYPE == fieldType)
							...
							field.set(entity, DateUtils.toDate(c, "yyyy-MM-dd HH:mm:ss"));
...
//		System.out.println("类型:"+typeName);
		switch (typeName) {
		case "java.util.Date":
			result = DateUtils.toString((Date)vo, "yyyy-MM-dd HH:mm:ss");
			break;
	...
	public static boolean vaildDate(String datestr, String formatstr) {
		...
		try {
			...
			// 成功：true ;失败:false
			System.out.println("日期 ["+datestr+"] 是否满足 [" + formatstr+"] 结果:"+dateflag);
		}
		return dateflag;
	}
